...
def _safe_alias(a: str) -> str:
    # python variable safe-ish
    out = "".join([c if c.isalnum() or c == "_" else "_" for c in a])
...
def generate_pyspark_code(pipeline: Pipeline, sources: List[Source]) -> str:
    """Generate PySpark code (preview only; does not execute in the UI)."""
    ...
    # Final output target (commented write - client wants preview/export only)
    target = pipeline.output_fullname()
    final_alias = _safe_alias(pipeline.final_output_alias())
    if target and final_alias:
        lines.append("# --- Optional: write to Unity Catalog table (disabled by default) ---")
        lines.append(f"# {final_alias}.write.format('delta').mode('overwrite').saveAsTable({_q(target)})")
